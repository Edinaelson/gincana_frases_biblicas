...
namespace GincanaPassagensBiblicas.Services
{
    ...
    public class OllamaService : IGeminiService
    {
        ...
        private static readonly string[] LivrosBiblia = {
        ...
        public async Task<(string? Passagem, string? Contexto)?> AnalyzeFullAsync(string text)
        {
            try
            {
                ...
                var prompt = $@"O usuário citou uma frase bíblica. Escolha a referência exata entre os candidatos abaixo.
Frase: ""{text}""
...
{{ ""encontrou"": true, ""referencia"": ""Livro Capítulo:Versículo"" }}";

                var payload = new { model = ModelName, prompt = prompt, stream = false, format = "json", options = new { temperature = 0.0, num_predict = 100 } };
                var response = await _http.PostAsync("http://localhost:11434/api/generate", new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json"));
                
                if (!response.IsSuccessStatusCode) return (candidates.First().Reference + " - " + candidates.First().Text, "");

                var responseBody = await response.Content.ReadAsStringAsync();
                var jsonStr = ExtractJson(JsonSerializer.Deserialize<OllamaResponse>(responseBody)?.Response ?? "");
        ...
        private async Task LoadBibleToCache()
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (matchCap.Success)
                {
                    ...
                    if (LivrosBiblia.Any(l => l.Equals(bookName, StringComparison.OrdinalIgnoreCase)))
                    {
                        currentBook = bookName;
                        currentChapter = matchCap.Groups[2].Value;
                        continue;
                    }
                }
                ...
                {
                    currentVerse.Text += " " + trimmed;
                    currentVerse.CleanText = CleanString(currentVerse.Text);
                }
            }
            if (currentVerse != null) _bibleCache.Add(currentVerse);
        }
        ...
        private List<BibleVerse> FindBestMatches(string query, int count)
        {
            ...
            return (_bibleCache ?? new List<BibleVerse>())
                ...
                .Take(count).Select(x => x.Verse).ToList();
        }
